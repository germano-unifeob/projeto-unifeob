...
// FakeResponse estendendo http.Response corretamente
...
void main() {
  ...
  group('Teste de Integração - Integração1 (INT-CAD-LOG-ING-IA-001)', () {
    test('TC-INT-IA-001 - Login funciona corretamente', () async {
      final loginData = {'email': 'user@test.com', 'senha': '123456'};

      when(() => mockApi.login(loginData)).thenAnswer(
        (_) async => FakeResponse('{"token":"abc123","user_id":42}', 200),
      );
    ...
    test('TC-INT-IA-002 - Ingrediente cadastrado corretamente', () async {
    ...
    test('TC-INT-IA-005 - Integração completa sem erros', () async {
      final loginData = {'email': 'user@test.com', 'senha': '123456'};
      final ingredientes = [
